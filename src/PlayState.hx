...
import funkin.LoadingState;

import funkin.jit.script.*;
import funkin.game.jit.event.*;
...
class PlayState extends MusicBeatState {
	...
	override public function create()
	{
		...
		for (notetype in noteTypeMap.keys()) {
			...
			startHScriptOnFolder('custom_notetypes/' + notetype);
		}
		for (event in eventPushedMap.keys()) {
			startLuasOnFolder('custom_events/' + event + '.lua');
			startHScriptOnFolder('custom_events/' + event);
		}
	...
	public function startCountdown():Void {
		...
		if(callOnLuas('onStartCountdown', [], false) == FunkinLua.Function_Stop || callEvent('onStartCountdown', new Cancellable()).cancelled) return;
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer) {
			...
			if(callOnLuas('onCountdownTick', [], false) == FunkinLua.Function_Stop || callEvent('onCountdown', new CountdownEvent(swagCounter)).cancelled) return;
		...
		callHScript('onPostStartCountdown', []);
	}
	...
	override function openSubState(SubState:FlxSubState) {
		if(callOnLuas('onSubStateOpen', [SubState], false) == FunkinLua.Function_Stop || callEvent('onSubStateOpen', new StateEvent(SubState)).cancelled) return;
		...
		if (paused) {
			...
			for (char in chars) {
				if(char != null && char.colorTween != null) char.colorTween.active = false;
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (controls.PAUSE && startedCountdown && canPause && callOnLuas('onPause', [], false) != FunkinLua.Function_Stop && !callEvent('onGamePause', new Cancellable()).cancelled)
			openPauseMenu();
		...
		callHScript('postUpdate', [elapsed]);
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		var event:GameOverEvent = cast callEvent('onGameOver', new GameOverEvent(
		...
		GameOverSubstate.loopSoundName = event.gameOverSong;
		GameOverSubstate.deathSoundName = event.lossSFX;
		GameOverSubstate.endSoundName = event.retrySFX;
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && callOnLuas('onGameOver', [], false) != FunkinLua.Function_Stop && 
			!event.cancelled) {
			...
			openSubState(new GameOverSubstate(event.x, event.y, event.camX, event.camY));
			...
			callOnLuas('onPostGameOver', []);
			callHScript('onPostGameOver', []);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		if(callOnLuas('onEvent', [eventName, value1, value2]) == FunkinLua.Function_Stop || callEvent('onEvent', new EventGameEvent(eventName, value1, value2)).cancelled) return;
		switch(eventName) {
			case 'Dadbattle Spotlight':
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		var event:NoteMissEvent = cast callEvent('onPlayerMiss', new NoteMissEvent(
			daNote,
			true,
			- daNote.missHealth * healthLoss,
			0,
			daNote.animSuffix,
		...
		songMisses++;
		if (event.muteVocals) vocals.volume = event.missVolume;
		if(!practiceMode) songScore -= 10;

		totalPlayed++;
		RecalculateRating(true);

		if(event.character != null && !daNote.noMissAnimation && event.character.hasMissAnimations && !event.animCancelled) {
			var animToPlay:String = singAnimations[event.direction] + 'miss' + event.animSuffix;
	...
	function goodNoteHit(note:Note):Void {
		...
		var event:NoteHitEvent = new NoteHitEvent(
			note,
			note.gfNote && gf != null ? gf : boyfriend,
			true,
			daRating.score,
		...
		if (note == null || note.wasGoodHit || (cpuControlled && (note.ignoreNote || note.hitCausesMiss))) return;
		...
		if (!note.isSustainNote && !note.hitCausesMiss && note.ignoreNote) {
			combo += 1;
			...
			if (PlayState.isPixelStage) {
				event.ratingPrefix = 'pixelUI/';
				event.ratingSuffix = '-pixel';
			}

			rating.loadGraphic(Paths.image(event.ratingPrefix + daRating.image + event.ratingSuffix));
			...
			rating.setGraphicSize(Std.int(rating.width * daPixelZoom * event.ratingScale));
			comboSpr.setGraphicSize(Std.int(comboSpr.width * daPixelZoom * event.ratingScale));
			...
			for (i in seperatedScore) {
				var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(event.ratingPrefix + 'num' + Std.int(i) + event.ratingSuffix));
				...
				numScore.setGraphicSize(Std.int(numScore.width * event.numScale));
		...
		if(!note.noAnimation) {
			...
			event.character.playAnim(animToPlay + note.animSuffix, true);
			event.character.holdTimer = 0;
	...
	override function beatHit() {
		...
		if (curStage == 'spooky' && FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset) lightningStrikeShit();
		
		lastBeatHit = curBeat;

		setOnLuas('curBeat', curBeat); //DAWGG?????
		callOnLuas('onBeatHit', [curBeat]);
		callHScript('beatHit', [curBeat]);
	}
	...
	public function startHScriptOnFolder(file:String) {
		for (script in hscripts) {
			if(script.path == file) return false;
		}

		if(FileSystem.exists(Paths.hscript(file))) {
			var script:HScript = new HScript(Paths.hscript(file), this);
	...
	public function callHScript(event:String, args:Array<Dynamic>) {
		for (script in hscripts) {
			script.call(event, args);
		}
	}

	public function callEvent(func:String, event:Cancellable) {
		for (script in hscripts) {
			script.call(func, [event]);
			if (event.cancelled && !event.ignoreStops) break;
		}
		return event;
	}
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if (callOnLuas('onRecalculateRating', [], false) == FunkinLua.Function_Stop || callEvent('onRatingUpdate', new Cancellable()).cancelled) return;
